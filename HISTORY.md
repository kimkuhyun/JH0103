...
- PDF 생성 시 메모리 사용량 70% 감소
- 처리 속도 60초 → 10초로 단축 (83% 개선)
- 사이트별 메인 섹션 선택자 정교화
...
| 버전 | 처리 방식 | 응답 시간 | 메모리 사용 |
|------|-----------|-----------|-------------|
| v2.0 | 동기 (병합) | 60초 | 높음 |
| v3.0 | 비동기 (폴링) | 10초 | 낮음 |
| v4.0 | 비동기 (최적화) | 10초 | 매우 낮음 |
...
### 5.1 단기 목표
- 처리 시간 10초 → 5초 이내 단축
- 추가 채용 사이트 지원 (링크드인, 인디드 등)
- 사용자 피드백 기반 정확도 개선

### 5.2 중기 목표
- 더 큰 AI 모델 또는 GPT-4V 통합 검토
- 실시간 검증 및 재시도 로직 추가
- 분석 결과 수정 기능 제공
